...
{
    static readonly List<FoundItem> foundItems = [];
    ...
    static void Init(SectionParagraphs section, string text)
    {
        ...
        {
            if (item is not ParagraphTitle && item is not ParagraphTaskSheet && item is not ParagraphListOfReferences &&
                item is not ParagraphAppendix)
            {
    ...
    public static void SelectedItemChanged(RoutedPropertyChangedEventArgs<object> e, ViewModelDocument document,
        RTBox richTextBox, TreeView treeView)
    {
